...
Мини-дашборд для тестового задания `AI Tools Specialist`.
...
Без реальных ключей и аккаунтов нельзя автоматически приложить:

- публичную ссылку на `Vercel`
- реальный скриншот `Telegram`
- подтверждение фактической загрузки заказов в ваш `RetailCRM`

Но кодовая база полностью подготовлена для этих шагов.
